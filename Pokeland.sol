...
/**
 * Website: https://pokeland.app/
 * Gameplay: https://play.pokeland.app
 */ 
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RW = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ...
    uint256 public minDistribution = 10 * (10**18); // 10 Reward minimum auto send
    uint256 public minimumTokenBalanceForDividends = 90000 * (10**9); // user must hold 90000 token
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = RW.balanceOf(address(this));
        ...
        uint256 amount = RW.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            RW.transfer(shareholder, amount);
...
contract POKELAND is
    BASEToken("POKELAND", "PKL", 90000000 * (10**9), 9),
    SafeToken,
    LockToken
{
    using SafeMath for uint256;
    ...
    function setExcludeFee(address[] memory holders, bool exempt) external onlyOwner {
        for (uint8 i = 0; i < holders.length; i++) {
            excludeFee[holders[i]] = exempt;
        }
    }

    function setExcludeMaxTxn(address[] memory holders, bool exempt) external onlyOwner {
        for (uint8 i = 0; i < holders.length; i++) {
            excludeMaxTxn[holders[i]] = exempt;
        }
    }
